<link rel="stylesheet" href="assets/style.css?v=20260223-chatbot">
    ...
    <!-- Attention Bubble -->
    <div class="chat-attention-bubble" id="chatAttentionBubble" onclick="toggleChat()">
        <span class="chat-attention-text">
            <i class="fas fa-sun fa-spin" style="color:#f59e0b;margin-right:5px;font-size:13px;"></i>
            Message NOW!
        </span>
        <div class="chat-attention-tail"></div>
    </div>

    <style>
        .chat-attention-bubble {
            position: fixed;
            /* Desktop: FAB bottom(20px) + FAB height(~56px) + gap(10px) = 86px */
            bottom: 86px;
            right: 18px;
            background: #ffffff;
            color: #1a1a2e;
            ...
            animation: bubbleEntrance 0.6s ease forwards, bubblePulse 2.5s ease-in-out 1s infinite;
            transform-origin: bottom right;
        }
        .chat-attention-bubble.hidden {
            opacity: 0;
            pointer-events: none;
        ...
        @keyframes bubblePulse {
            ...
            50%       { transform: scale(1.05); box-shadow: 0 8px 28px rgba(245,158,11,0.35); }
        }
        .chat-attention-text::after {
            content: '';
            display: inline-block;
        }

        /* Mobile: FAB bottom(80px) + FAB height(~56px) + gap(10px) = 146px */
...
<script>
        ...
        const chatAttentionBubble = document.getElementById('chatAttentionBubble');

        // Auto-show bubble after 3 seconds if chat not yet opened
        setTimeout(() => {
            if (!chatInited && chatAttentionBubble) {
                chatAttentionBubble.style.display = 'block';
            }
        }, 3000);
        ...
        function toggleChat() {
            ...
            if (isOpen && chatAttentionBubble) chatAttentionBubble.classList.add('hidden');
