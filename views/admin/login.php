...
$msg = "";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $conn->prepare("SELECT id, firstName, lastName, password, 'admin' as role FROM admin WHERE username=? LIMIT 1");
        $stmt->bind_param("s", $username);
        $stmt->execute();
        $stmt->store_result();

        // If not found in admin, check client table
        if ($stmt->num_rows == 0) {
            $stmt->close();
            $stmt = $conn->prepare("SELECT id, firstName, lastName, password, 'client' as role FROM client WHERE username=? LIMIT 1");
            $stmt->bind_param("s", $username);
            $stmt->execute();
            $stmt->store_result();
        }
